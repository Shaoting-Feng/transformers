...
def create_causal_mask(
    ...
    if past_key_values:
        ...
        for i in range(len(past_key_values)):
            tmp_early_exit, tmp_attention_mask, tmp_packed_sequence_mask, tmp_kv_length, tmp_kv_offset = _preprocess_mask_arguments(
                config, input_embeds, attention_mask, cache_position, past_key_values, position_ids, i
            )
            if tmp_kv_length > kv_length:
                early_exit, attention_mask, packed_sequence_mask, kv_length, kv_offset = (
                    tmp_early_exit,
                    tmp_attention_mask,
                    tmp_packed_sequence_mask,
                    tmp_kv_length,
                    tmp_kv_offset,
                )
    else:
